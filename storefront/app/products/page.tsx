...
const PLAN_FEATURES: Record<string, string[]> = {
  ...
  business: [
    'Lookups illimités',
    'Tous les 16 champs append 🇲🇦',
    'Wilaya + Code Wilaya',
    'Commune + Code Commune',
    'Zone CIN + Circonscription',
    'Tribunal Compétent',
    'Académie Régionale',
    'Batch CSV illimité',
    'Webhooks & callbacks',
...
const faqs = [
  {
    ...
    a: "Un lookup correspond à une requête de géocodage : vous envoyez une adresse marocaine, l'API retourne les coordonnées GPS + les champs append (wilaya, commune, zone CIN, etc.). Chaque adresse traitée = 1 lookup.",
  },
  {
    q: 'Quelles données marocaines sont incluses dans les champs append ?',
    a: "Wilaya, code wilaya, province/préfecture, commune, code commune (HCP), arrondissement, région administrative, code régional, code postal, secteur postal, zone CIN, circonscription électorale, tribunal compétent, académie régionale, centre de santé.",
  },
  {
    q: "D'où viennent les données géographiques ?",
    a: "Nous utilisons la Base Nationale des Adresses du Haut-Commissariat au Plan (HCP), complétée par OpenStreetMap Maroc et les données officielles du Ministère de l'Intérieur pour les découpages administratifs.",
  },
  {
    q: "L'API supporte-t-elle les adresses en arabe ?",
    a: "Oui. L'API accepte les adresses en français, en arabe (script arabe) et en translittération latine. La normalisation multilingue est intégrée dans le moteur de parsing.",
  },
  {
    q: 'Les lookups non utilisés sont-ils reportés ?',
    a: "Non, les lookups se réinitialisent chaque mois et ne sont pas cumulables. Pour des besoins variables, contactez-nous pour un plan Enterprise sur-mesure.",
  },
  {
    q: 'Puis-je géocoder en batch (fichier CSV) ?',
    a: "Oui. Le plan Free permet 1 batch/jour (max 500 lignes). Le plan Business permet des batchs illimités jusqu'à 10 000 lignes par fichier. Le plan Enterprise est sans limite.",
  },
]
  ...
  return (
    <div className="border-b border-slate-100 last:border-0">
      <button
        ...
        className="flex items-center justify-between w-full py-4 text-left text-sm font-semibold text-slate-800 hover:text-red-600 transition-colors"
      >
        {q}
  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border transition-all ${
        highlight
          ? 'bg-red-600 border-red-500 shadow-2xl shadow-red-500/20 scale-[1.03] text-white'
          : 'bg-white border-slate-200 shadow-sm hover:border-red-200 hover:shadow-md'
      }`}
    >
      {highlight && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-white text-red-600 text-xs font-bold rounded-full uppercase tracking-widest shadow-sm">
          ⭐ Populaire
        </div>
      )}

      <div className="p-8 flex-1">
        <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${highlight ? 'text-red-200' : 'text-slate-400'}`}>
        ...
        <div className="flex items-end gap-1 mt-4 mb-2">
          {priceFormatted ? (
            <>
              ...
              <span className={`text-sm mb-1.5 ${highlight ? 'text-red-200' : 'text-slate-400'}`}>/mois</span>
            </>
          ) : (
        ...
        {product.description && (
          <p className={`text-sm mb-6 leading-relaxed ${highlight ? 'text-red-100' : 'text-slate-500'}`}>
            {product.description}
          </p>
        )}
        ...
        <ul className="space-y-2.5 mb-8">
          {features.map((f) => (
            <li key={f} className={`flex items-center gap-2.5 text-sm ${highlight ? 'text-red-50' : 'text-slate-600'}`}>
              <CheckCircle2 className={`h-4 w-4 shrink-0 ${highlight ? 'text-white' : 'text-red-500'}`} />
              {f}
            </li>
          ))}
        </ul>
      ...
      <section className="bg-gradient-to-b from-slate-950 via-red-950/20 to-slate-900 text-white">
        <div className="container-custom py-20 lg:py-28 text-center">
          ...
          <h1 className="text-4xl lg:text-6xl font-heading font-bold text-balance leading-tight mb-6">
            ...
            <span className="text-red-400">sans surprise</span>
          </h1>
          <p className="text-slate-300 text-lg max-w-xl mx-auto mb-10">
            Commencez gratuitement avec 2 500 lookups. Passez au niveau supérieur selon vos besoins.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 text-sm text-slate-400">
            {[
              ...
              { icon: <Globe className="h-4 w-4 text-red-400" />, label: '12 régions' },
            ].map(({ icon, label }) => (
        ...
        <div className="container-custom py-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
            <div>
              ...
              <p className="text-slate-400 text-sm">2 500 lookups/mois · Wilaya & Commune inclus · Aucune carte bancaire</p>
            </div>
            <Link
              href="/auth/register"
              className="shrink-0 px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white text-sm font-bold rounded-xl transition-colors"
            >
        ...
        <div className="container-custom">
          {isLoading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-[600px] bg-slate-200 rounded-2xl animate-pulse" />
              ))}
            </div>
            ...
            <div className="text-center py-20 text-slate-400">
              ...
              <Link href="/contact" className="mt-4 inline-block text-red-600 hover:underline text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="text-center px-4 py-4 text-slate-700 font-semibold">Gratuit</th>
                  <th className="text-center px-4 py-4 text-red-600 font-bold bg-red-50">Business</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {[
                  ...
                  <tr key={feature} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-center text-slate-800 font-semibold bg-red-50/40">{business}</td>
